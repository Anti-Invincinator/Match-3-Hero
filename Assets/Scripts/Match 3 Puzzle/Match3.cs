...
public class Match3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < finishedUpdating.Count; i++)
        {
            ...
            {
                int moveType = 1;                  //To fetch the node value
                int moveAmount = 0;                //To fetch the multiplier 3, 4, 5 tile match
                ...
                foreach (Point pnt in connected) //Remove the node pieces connected
                {
                    ...
                    if (nodePiece != null)
                    {
                        moveType = nodePiece.value; //Execute the move according to node type
                        nodePiece.gameObject.SetActive(false);
                        dead.Add(nodePiece);                      //Deactivates node piece and drops the kill piece
                    }
                    node.SetPiece(null);
                    moveAmount++;
                }
                MoveManager.CheckMoveType(moveType, moveAmount);     //Execute the move
                ApplyGravityToBoard();
            }
...
public class Node
{
    public int value; //0 = blank, 1 = attack 1, 2 = spell, 3 = atk up, 4 = atk speed, 5 = attack 2, -1 = hole
    public Point index;
    NodePiece piece;
